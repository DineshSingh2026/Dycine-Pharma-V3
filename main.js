...
(() => {
  ...
  const selectors = ['#about', '#products', '#capabilities', '#partners'];
  ...
  // Pause off-screen non-hero videos at load so nothing decodes until needed
  window.addEventListener('load', () => {
    sections.forEach(s => {
      const r = s.getBoundingClientRect();
      const visible = r.bottom > -160 && r.top < window.innerHeight + 160;
